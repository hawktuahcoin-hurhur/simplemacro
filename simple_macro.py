...
class SimpleMacroGUI:
    ...
    def __init__(self):
        ...
        # Only play hotkey remains
        self.play_hotkey = "F6"
        # Discord webhook settings (defaults)
    ...
    def _setup_ui(self):
        ...
        # Button frame - Row 2 (Image Search & Settings)
    ...
    def _open_settings(self):
        ...
        def save_settings():
            ...
            self.status_label.config(text=f"Settings: {self.loop_count}x loop, {self.playback_speed:.1f}x speed | Play: {self.play_hotkey}")
            dialog.destroy()
    ...
    def _on_record_click(self, x, y, button, pressed):
        ...
        print(f"Recording click: {button} {'pressed' if pressed else 'released'} at ({x}, {y}) at {timestamp:.2f}s")
    ...
    def _on_record_key_press(self, key):
        ...
        if not self.record_options['keys'].get():
            return
        # Filter out the record hotkey itself to avoid recording it
        try:
            key_name = None
            if hasattr(key, 'name'):
            ...
            if key_name and key_name == self.record_hotkey.upper():
                print(f"Ignoring record hotkey: {key_name}")
        ...
        print(f"Recording key press: {key_str} at {timestamp:.2f}s")
        self.recorded_events.append({
            'type': 'key_press',
            'timestamp': timestamp,
            'key': key_str
    ...
    def _save_recording_to_file(self):
        ...
        if not self.recorded_events:
            messagebox.showinfo("Save Recording", "No recording events to save.")
            return
        ...
        if file_path:
            try:
                ...
                messagebox.showinfo("Save Recording", f"Recording saved to {file_path}")
            except Exception as e:
                messagebox.showerror("Save Error", f"Failed to save recording: {e}")

    def _convert_recording_from_file(self):
        """Load a recording from a file and convert it to steps"""
        import json
        from tkinter import filedialog
        ...
        if file_path:
            try:
                ...
                self._convert_events_to_steps()
                messagebox.showinfo("Convert Recording", f"Converted {len(events)} events to {len(self.steps)} steps.")
                self._update_steps_display()
            except Exception as e:
                messagebox.showerror("Convert Error", f"Failed to convert recording: {e}")
    ...
    def _convert_events_to_steps(self):
        ...
        if not self.recorded_events:
            ...
            return
        print(f"Starting conversion of {len(self.recorded_events)} events...")
            ...
            if event['type'] == 'mouse_click' and event['pressed']:
                ...
                for j in range(i + 1, len(events)):
                    if (events[j]['type'] == 'mouse_click' and 
                        ...
                        not events[j]['pressed'] and
                        j not in processed_indices):
                        release_idx = j
                        break
                if release_idx is not None:
                    release_event = events[release_idx]
                    ...
                    if hold_duration > 0.3:  # Considered a hold if > 300ms
                        ...
                        print(f"  Created hold step: {event['button']} for {hold_duration:.2f}s")
                    else:
                        new_steps.append({
                            'action': 'click',
                            'key': event['button'],
                        ...
                        print(f"  Created click step: {event['button']}")
                    # Mark both press and release as processed
                    ...
                    print(f"  Created click step (no release): {event['button']}")
                    processed_indices.add(i)
                ...
                release_idx = None
                for j in range(i + 1, len(events)):
                    if (events[j]['type'] == 'key_release' and 
                        events[j]['key'] == key and
                        j not in processed_indices):
                        release_idx = j
                        break
                if release_idx is not None:
                    ...
                    if hold_duration > 0.3:  # Considered a hold
                        ...
                        print(f"  Created key hold step: {key} for {hold_duration:.2f}s")
                    else:
                        new_steps.append({
                            'action': 'click',
                            'key': key,
                        ...
                        print(f"  Created key click step: {key}")
                    # Mark both press and release as processed
                    processed_indices.add(i)
                    processed_indices.add(release_idx)
                else:
                    ...
                    print(f"  Created key click step (no release): {key}")
                    processed_indices.add(i)
                last_timestamp = event['timestamp']
                ...
                print(f"  Created mouse move step to ({event['x']}, {event['y']})")
                processed_indices.add(i)
                last_timestamp = event['timestamp']
                ...
                print(f"  Created scroll step: {scroll_amount}")
                processed_indices.add(i)
                last_timestamp = event['timestamp']
            i += 1
        print(f"Conversion complete: {len(new_steps)} steps created")
    ...
    def _send_discord_webhook(self, content=None):
        """Send a Discord webhook asynchronously. If `content` is None an empty message is sent.

        The webhook will only be sent if `self.discord_webhook_url` is set and
        `self.discord_webhook_enabled` is True.
        """
        if not getattr(self, 'discord_webhook_url', None) or not getattr(self, 'discord_webhook_enabled', False):
            return
    ...
    def _notify_loop_complete(self, loop_number: int):
        """Called when a macro loop completes - posts an embed with a screenshot."""
        if not getattr(self, 'discord_webhook_url', None) or not getattr(self, 'discord_webhook_enabled', False):
            return
        ...
        def _do_notify():
            try:
                ...
                files = {'file': ('screenshot.png', bio, 'image/png')}

                _requests.post(self.discord_webhook_url, data={"payload_json": _json.dumps(payload)}, files=files, timeout=10)
            except Exception as e:
